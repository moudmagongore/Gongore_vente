...
class FournisseurDetailView extends GetView<FournisseurDetailController> {
  const FournisseurDetailView({super.key});
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          ...
          child: Obx(() {
            ...
            return Column(
              children: [
                _Header(fournisseur: f, controller: controller),
                Expanded(
                  child: TabBarView(
                    children: [
                      _ApprosTab(controller: controller),
...
class _Header extends StatelessWidget {
  ...
  final FournisseurDetailController controller;
  const _Header({required this.fournisseur, required this.controller});
...
class _ApprosTab extends StatelessWidget {
  final FournisseurDetailController controller;
  const _ApprosTab({required this.controller});

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      final list = controller.approvisionnements;
      if (list.isEmpty) {
        return _EmptyState(
          icon: Icons.local_shipping_outlined,
          message: 'Aucun approvisionnement pour ce fournisseur.',
        );
      }
              ...
              children: [
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    if (annulee)
                      const _MiniBadge(label: 'Annulée', color: Colors.red)
                    else if (hasCredit)
                      const _MiniBadge(
                ...
                if (hasCredit) ...[
                  const SizedBox(height: 2),
                  Text(
                    'Reste : ${Fmt.number(appro.resteAPayer)}',
                    style: const TextStyle(
                      fontSize: 11,
...
String _imputationsLabel(ReglementFournisseurModel r) {
  if (r.imputations.isEmpty) return '';
  final parts = r.imputations.map((i) {
    final num = i.numero ?? i.approId.substring(0,
        i.approId.length < 6 ? i.approId.length : 6);
